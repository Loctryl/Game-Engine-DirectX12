...
#include "Shaders/UIShaders/UIElementShader.h"
...
void RenderManager::CreateGeometries()
{
    ...
    mGeometries.push_back(CreateGeometry(losVertices, _countof(losVertices), losIndices, _countof(losIndices),
                                         "Losange"));
    ...
    mGeometries.push_back(CreateGeometry(quadVertices, _countof(quadVertices), quadIndices, _countof(quadIndices),
                                         "Quad"));
    ...
    mGeometries.push_back(CreateGeometry(cubeVertices, _countof(cubeVertices), cubeIndices, _countof(cubeIndices),
                                         "Cube"));
...
void RenderManager::CreateShaders()
{
    ...
    UIElementShader* uiShad = new UIElementShader();
    mDirectX->CreateShader(uiShad, L"ShadersHlsl\\UIElementShader.hlsl");
    ...
    for (auto obj : mComponents)
    {
        ...
        if (std::abs(Pos.x - cam.x) > KILLBOX ||
            ...
            if (obj->mIsDestructible)
                obj->mGameObject->mToDestroy = true;
    }
}
...
MeshGeometry* RenderManager::GetLosMesh() const { return mGeometries[0]; }

MeshGeometry* RenderManager::GetSquareMesh() const { return mGeometries[1]; }

MeshGeometry* RenderManager::GetCubeMesh() const { return mGeometries[2]; }

MeshGeometry* RenderManager::GetSphereMesh() const { return mGeometries[3]; }

MeshGeometry RenderManager::GetSkyMesh() const { return *mGeometries[4]; }

Shader* RenderManager::GetShader(SHAD index) const { return mShaders[index]; }

Shader* RenderManager::GetSkyShader() const { return mShaders.back(); }
...
MeshGeometry* RenderManager::CreateGeometry(Vertex vertex[], int numVer, uint16_t index[], int numInd, string name) const {
    return mDirectX->CreateGeometry(vertex, numVer, index, numInd, name);
}

void RenderManager::ResetShaders() const
{
    for (int i = 0; i < mShaders.size(); i++)
        mShaders[i]->Reset();
}